...
require_once(dirname(__FILE__) . '/DeviceIdManager.php');
require_once(dirname(__FILE__) . '/SecurityData.php');
...
class LiveIDManager {
	...
	$cipherValue =  $matches[1];
        ...
        if( isset ($cipherValues) && $cipherValues->length>0 ){
            ...
            error_log("Error during get security password");
            return null;
        }
 ...
 //       curl_setopt($cURLHandle, CURL_SSLVERSION_SSLv3, 3);
        
        $response = curl_exec($cURLHandle);
        
        $getinfoxx=curl_getinfo($cURLHandle);
        $errnoxx=curl_errno($cURLHandle);
        $errorxx=curl_error($cURLHandle);
